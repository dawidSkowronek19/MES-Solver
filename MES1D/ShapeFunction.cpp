...
ShapeFunction::ShapeFunction(ConfigParameters &config) : m_config(config)
{
...
{
    double phi=1.0;
    double elementStep=2.0/m_config.shapeFunctionDeg; //   |----|----|----| p+1 nodes, dim(phi)=p
    double ksi_i = -1.0 + elementStep*i;
    for (int j=0; j<m_config.shapeFunctionDeg+1; j++)
    {
        if (i==j) continue;
...
double ShapeFunction::Rphi_1D_deriv(double ksi, int i)
{
    ...
    double elementStep=2.0/m_config.shapeFunctionDeg; //   |----|----|----| p+1 nodes, dim(phi)=p
    double ksi_i = -1.0 + elementStep*i;
    for (int k=0; k<m_config.shapeFunctionDeg+1; k++)
    {
        if (k==i) continue;
...
int ShapeFunction::get_deg() {return m_config.shapeFunctionDeg;}